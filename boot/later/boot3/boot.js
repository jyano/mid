...
function modal(){
    $.mF = $.modalFooter = function () {
        var d = $.dK('modal-footer')
...
function container(){
    ...
    function otherContainerDivs() {
        ...
        $.cp = $.caption = function (cap) {
            return $.dK("caption").A($.h4(cap))
        }
    }
...
function notUsedCurrently(){
    $.fG1 = function (a) {
        alert('$.fG1!')
        //"<div class="form-group"></div>"
        return _a($.dK('form-group'), G(arguments))
    }
    $.gl = function () {//not used currently
        $l('------- boot.js $.gl')
        var g = G(arguments), str, sp
