	...
	while (qu.length) {
		...
		_.t(n = vec.length, function (i) {
			...
			///////////////////////////////////////////
				...
				isVex = false; minLen = 1000000000000000000000000
				_.t(n, function (j) {
					if (j != i1 && j != i2) {
		...
		if (isVex) {vexPols.push(  qu.shift()  )}
	}
...
$sep = Box2D.sep =function(body, vs, scale){
    ...
    vs= _.map(vs,function(v){
	    v=V(v);
	    return [v.x, v.y]
    })

    return U(body)? Separate :
        Separate(body, vs, scale)
