...
function voteImg($class, $src){ ?>

   <img src="<?= $src ?>"  class="<?= $class ?>"  height="20" border="0">
...
function toTopicUrl($topic){
   ...
   return "/page.php?topic=" . $topic;
}


function result($queryString){
...
$untrim_topic = " ". $topic . " ";
$rank = $hyperRow['score'];
if (strlen($topic) > 2 and strtolower($topic) != strtolower($exclude) and
    stristr($untrim_string, $untrim_topic) != false and $rank > $matchrank)
{$match = $topic; $matchrank = $rank;}
}
$url = "http://www.dirtpage.com/page.php?topic=" . str_replace(" ", "+", $match); 
$link = "<a class=\"card\" href = \"$url\">" . $match . "</a>";
$hyper_string = str_ireplace($match, $link, $string);
return strtolower($hyper_string);}
