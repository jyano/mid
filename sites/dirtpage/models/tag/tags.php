...
<input type="hidden" name="act" value="post_tag">
 ...
 $i=0
 while(  $row = "SELECT * FROM tags WHERE topic = '$topic' ORDER BY score DESC"   ){$i++;


echo "<span class=\"smallyellow\">$i.&nbsp;&nbsp;</span>";
echo  "<span class = \"message\">" . ($row['tag']) . "</span>"   .  "<span class = \"t\">".  " (" . $row['score']. ") </span>&nbsp; ";
if( $odd = $row['score']%2 ) {$pic1 = "/graphics/up.png"; $class1 = "up"; $pic2 = "/graphics/down.png"; $class2 = "down";}
else {$pic2 = "/graphics/up.png"; $class2 = "up"; $pic1 = "/graphics/down.png"; $class1 = "down";   }?>
...
$.get("/resources/datacalls.php", {topic: <?php echo "\"$topic\"" ?>, new_tag: document.post_tag.new_tag.value, act: "post_tag"}, function() {
$.get("/divfillers/tags.php", { topic: <?php echo "\"$topic\"" ?>}, function(data) {
  $('#holder').html(data);}); }); });

$('.up').click(function() {
$.get("/resources/datacalls.php", {tag_id: this.name, act: "tag_up"}, function() {
$.get("/divfillers/tags.php", {topic: <?php echo "\"$topic\"" ?>, message: <?php echo "\"$message_id\"" ?>}, function(data) {
$('#holder').html(data);}); });
});
...
$.get("/resources/datacalls.php", {tag_id: this.name, act: "tag_down"}, function() {
