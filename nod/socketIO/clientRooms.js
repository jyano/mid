...
k.on('roomUpdate', function(upd ){
    ...
    if( room ){ room.updateUsersDiv(  upd .users)  }
...
$dU = du=function(){var u=c0().toDataURL();socket.emit('du',u);return u}
...
$emitButton   =bte=function(buttonText ,toEmit){
    ...
    return $.bt(buttonText,
        ...
        function(){  k.emit(toEmit || buttonText)  }
...
k.on('alert',  function(a){alert(a)})
k.on('log',function(d){  $l(d)  })
k.on('dir',function(d){$l('SERVER: %j',d)})
k.on('pop',function(e){pop(e)})
