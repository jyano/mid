	...
	function locApps() {
		...
		BBTODOS = function () {
			...
			_.x(
					bb.lS.prototype, {
						s: function () {
							$ls.s(this.n, this.d.join(','))
						},  // Save the cur state of the lS
						c: function (m) {
							if (!m.id) {
								m.id = guid();
						...
						f: function (m) {
							return this.j($ls.g(this.n + "-" + m.id))
						},  // Retrieve a m from `this.d` by id.
						a: function () {
							return _(this.d).chain().m(function (id) {
								return this.j($ls.g(this.n + '_' + id))
							}, this).cp().v()
						}, // Ret [ stored mds ]
						x: function (m) {
							if (m.isNew()) {
							...
							;
							$ls.rm(this.n + "-" + m.id)
							this.d = _.rj(this.d, function (id) {
								return id === m.id.tS()
							})
							this.s();
							return m
			...
			Td = $M(
					{
						d: function () {
							return {tt: 'empty..', ord: tdL.nOrd(), done: false}
						}, i: function () {
						if (!this.g('tt')) {
							this.s({tt: this.df().tt})
						}
					}, gg: function () {
						this.s({done: !this.g('done')})
					}
					}
			)
			TdVw = $V(
					{
						...
						ggDone: function () {
							this.md.gg()
						},   // Toggle the `"done"` state of the md.
						ed: function () {
							this.q.K("editing");
							this.ip.focus()
			...
			TdLs = $Cl(
					{
						...
						}, // Filter down the ls of all finished tds
						remain: function () {
							return this.wo.apply(this, this.done())
						}, // Filter down the ls to only unfinished tds    //without
						nOrd: function () {
							return !this.length ? 1 : this.last().g('ord') + 1
						}, //   gens the next order num   for new items
						compor: function (td) {
							return td.g('ord')
						}// tds sorted by orig insert ord  // tds kept sorted in seq ord  // desp being saved by unord GUID in db
					}
			);
			tdL = TdLs()
			ApVw = $V(
					{// ApVw is top-lvl// Instead of geng new el,// bind to the existing skel  of // the Ap  already present in the HT
						...
						e: {
							...
							"$ #toggle-all": "toggleAllComplete"
						},// Delegated events for:// creating new items, clearing completed ones.
						i: function () {
						...
						a1: function (td) {
							$("#td-ls").A(TdVw({md: td}).r().q)
						},  // Add a single td i  to the ls // by cr  a vw for it, and appending its el  to the `<ul>`.
						aA: function () {
							TdL.e(this.A1, this)
						},
