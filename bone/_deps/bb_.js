...
addUnderscoreMethods = function(Class, mets, att){
    _.e(mets, function(len, met) { if (_[met]) Class.prototype[met] = addMethod(len, met, att) }) }
cb = function(itee, instance) {return F(itee)?  iteratee:
        (O(itee) && !instance._isModel(itee))? modelMatcher(itee):
        S(itee)? function(md) { return md.get(itee) }:
        itee }
modelMatcher = function(attrs) {var matcher = _.mt(attrs); return function(model) { return matcher(model.attributes) } }
extend = function(protoProps, staticProps) {var parent = this,
...
wrapError = function(md,op){var z = op.error; op.error = function(resp) {if (z) {z.call(op.context, md, resp, op)};
...
function evens(){
    ...
    Events = Bb.Events = {}
    Events.trigger =  function(n){var Ev=this
        if (!Ev._events) {return Ev}
    ...
    Events.unbind = Events.off; _.x(Bb, Events)

    function eventsApi(iteratee, events, name, callback, opts) {var i = 0, names
    ...
    function onceMap(map, name, callback, offer){if (callback) {var once = map[name] = _.o(function() {
                offer(name, once);
...
function modl(){
    Model = Bb.Model = function (attributes, op) {var attrs = attributes || {};
        op || (op = {});
    ...
    _.x(Model.prototype, Events, {
        ...
        destroy: function (op) {
            ...
            if (this.isNew()) {_.do(op.success)}
            else { wrapError(this, op); xhr = this.sync('delete', this, op) }
            if (!wait) {destroy()}
            return xhr
        },
    ...
    _.x(Collection.prototype, Events, {
        model: Model,
        initialize: function(){},
        toJSON: function (op){return this.map(function(md){return md.toJSON(op) }) },
        sync: function () {return Bb.sync.apply(this, arguments)},
        add: function (mds, op) {
    ...
    _.x(View.prototype, Events, {
        ...
        _ensureElement: function (){
            if (!this.el) {
                ...
                if (this.id) {atts.id = _.rs(this, 'id')}
                if (this.className) {atts['class'] = _.rs(this, 'className')};
                this.setElement(this._createElement(_.rs(this, 'tagName')));
                this._setAttributes(atts) }
            else { this.setElement(_.rs(this, 'el')) }
        },


        _createElement: function (tagN){return document.createElement(tagN) },
        _setAttributes: function (atts) {this.$el.attr(atts)},
        ...
        _removeElement: function(){ this.$el.rm()},
        ...
        _setElement: function(el){
            this.$el = el instanceof Bb.$ ? el : Bb.$(el); this.el = this.$el[0]
            this.q= this.$el
        },


        delegate: function (evN, sel, listener) {this.$el.on(evN + '.delegateEvents' + this.cid, sel, listener); return this },
        undelegate: function (evN, sel, listener) {this.$el.off(evN + '.delegateEvents' + this.cid, sel, listener); return this },
        delegateEvents: function (ev) {ev || (ev = _.rs(this, 'ev'));
            if (!ev) return this; this.undelegateEvents(); for (var key in ev) {var met = ev[key];
                if (!F(met)) {met = this[met]}; if (!met) {continue};
                var match = key.match(delegateEventSplitter); this.delegate(match[1], match[2], _.b(met, this)) }
            return this },
