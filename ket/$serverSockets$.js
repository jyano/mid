...
KK.on('connection', function (k) {
	...
	function smitha(){
		...
		k.on('disconnect', function(da){
			$l('disconnect')
			if(!k.nickname){$l('no nickname'); return}
			smitha_Nicknames.splice(smitha_Nicknames.indexOf(k.nickname),1)
			updateNicknames()
		})

		function updateNicknames(){io.sockets.emit('smitha-usernames', smitha_Nicknames)}
	k.on('smitha-send-message', function(da){
		$l('smitha message: ' + da)
		io.sockets.emit('smitha-new-message', {msg: da, nick: k.nickname })
	})
 
	k.on('smitha-new-user', function(da, fn){
		$l('smitha-new-user: ' + da)
		$l(smitha_Nicknames)
		if( smitha_Nicknames.indexOf(da)!=-1 ){
			$l('name used')
			fn(false)
		} else {
			$l('name not used')
			fn(true)
			k.nickname = da
			smitha_Nicknames.push( k.nickname )
			updateNicknames()
		}
		
		$l('current users: ' + smitha_Nicknames.length)
		$l(smitha_Nicknames)
	})}
...
function oldGame(){
	...
	$sendServerMap = snSM = function () {
		m.guy.find(function (z, m) {
			k.b.e('mU', m)
			k.e('mU', m)
		})
	}
	$move = move = function (a, b, d) {
		if (d == 'u') {
			a.y -= b
		}
		;
		...
		;
		return a
	}//Gs=[{n:'c',x:300,y:200},{n:'c',x:300,y:200}]
	o('eG2', function () {
		m.guy.findOne(
				{n: s.u}, function (z, g) {
					if (!(g)) {
						l('nP!:' + s.u + '!');
						m.guy.create(Guy(s), function (z, g) {
							if (z) {
								n(z)
							}
							k.b.e('nPlr', g)
